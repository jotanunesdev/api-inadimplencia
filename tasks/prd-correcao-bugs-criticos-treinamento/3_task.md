...
- [ ] 3.1 Capturar `useSharePoint = isSharePointEnabled()` no início de `saveCollectiveTurmaEvidencias`
- [ ] 3.2 Usar `useSharePoint` capturado na decisão de cleanup (não chamar `isSharePointEnabled()` novamente)
- [ ] 3.3 Mover lógica de cleanup para bloco `finally` garantindo execução sempre
- [ ] 3.4 Adicionar try-catch individual em cada operação de cleanup com log de falha
- [ ] 3.5 Testar cenário: falha no meio do upload → arquivos temporários são limpos
...
- [ ] Estado do SharePoint é capturado no início da operação
- [ ] Cleanup de arquivos temporários executa sempre (sucesso ou falha)
- [ ] Cleanup de arquivos movidos usa estado capturado (não estado atual)
- [ ] Falhas de cleanup são logadas para investigação
- [ ] 100% de arquivos temporários removidos após falhas simuladas

## Testes da Tarefa

- [ ] **Teste de Unidade 1:** Simular falha no meio do upload, verificar que arquivos temporários são removidos
- [ ] **Teste de Unidade 2:** Simular mudança de config SharePoint durante upload, verificar que cleanup usa estado inicial
- [ ] **Teste de Unidade 3:** Simular falha no cleanup, verificar que erro é logado e não quebra a aplicação
- [ ] **Teste de Integração:** Upload de evidência com falha simulada no SharePoint, verificar que temp files são limpos
