...
async function fetchDataset(name, options = {}) {
    ...
    if (options.constraints?.length) {
        appendArrayParams(url.searchParams, 'constraintsField', options.constraints.map((constraint) => constraint._field ?? constraint.fieldName ?? ''), true);
        appendArrayParams(url.searchParams, 'constraintsInitialValue', options.constraints.map((constraint) => constraint._initialValue ?? constraint.initialValue ?? ''), true);
        appendArrayParams(url.searchParams, 'constraintsFinalValue', options.constraints.map((constraint) => constraint._finalValue ?? constraint.finalValue ?? ''), true);
        appendArrayParams(url.searchParams, 'constraintsType', options.constraints.map((constraint) => resolveConstraintType(constraint._type ?? constraint.type ?? DEFAULT_CONSTRAINT_TYPE)), true);
    }
