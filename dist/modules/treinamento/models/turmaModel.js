...
exports.listTurmaParticipants = listTurmaParticipants;
exports.saveTurmaEncerramentoEvidencias = saveTurmaEncerramentoEvidencias;
exports.getCollectiveTrainingHoursMonthlySummaryLast12Months = getCollectiveTrainingHoursMonthlySummaryLast12Months;
...
async function ensureTurmaEvidenciaSchema() {
  ...
  `);
    ...
    if (!row?.DURACAO_COL || !row?.EVID_TABLE_ID) {
        ...
        error.code = "TURMA_EVIDENCIA_SCHEMA_MISSING";
        throw error;
    }
}
async function createTurma(input) {
    ...
    const turmaId = (0, crypto_1.randomUUID)();
    const startedAt = input.iniciadoEm ?? new Date();
    const status = "iniciada";
    try {
        await new db_1.sql.Request(transaction)
            .input("ID", db_1.sql.UniqueIdentifier, turmaId)
            .input("NOME", db_1.sql.NVarChar(200), input.nome)
            .input("STATUS", db_1.sql.VarChar(30), status)
            .input("CRIADO_POR", db_1.sql.VarChar(100), input.criadoPor ?? null)
            .input("INICIADO_EM", db_1.sql.DateTime2, startedAt)
            .query(`
      ...
      `);
        for (const cpf of input.participantesCpf) {
            await new db_1.sql.Request(transaction)
                ...
                .input("TURMA_ID", db_1.sql.UniqueIdentifier, turmaId)
...
async function listTurmas(search) {
    const pool = await (0, db_1.getPool)();
    const request = pool.request();
    let where = "";
    if (search?.trim()) {
        request.input("SEARCH", db_1.sql.NVarChar(250), `%${search.trim()}%`);
        where = `
...
async function listTurmaParticipants(turmaId) {
    const pool = await (0, db_1.getPool)();
    const result = await pool
        .request()
        .input("TURMA_ID", db_1.sql.UniqueIdentifier, turmaId)
...
async function saveTurmaEncerramentoEvidencias(input) {
    ...
    try {
        await new db_1.sql.Request(transaction)
            ...
            .input("DURACAO_TREINAMENTO_MINUTOS", db_1.sql.Int, input.duracaoTreinamentoMinutos)
            .input("FINALIZADO_EM", db_1.sql.DateTime2, input.finalizadoEm ?? new Date())
            .query(`
      ...
      `);
        ...
        for (let index = 0; index < input.evidencias.length; index += 1) {
            ...
            await new db_1.sql.Request(transaction)
                .input("ID", db_1.sql.UniqueIdentifier, (0, crypto_1.randomUUID)())
                .input("TURMA_ID", db_1.sql.UniqueIdentifier, input.turmaId)
                .input("ARQUIVO_PATH", db_1.sql.NVarChar(1000), evid.arquivoPath)
                .input("CRIADO_POR", db_1.sql.VarChar(100), input.criadoPor ?? null)
                .input("ORDEM", db_1.sql.Int, index + 1)
                .query(`
...
async function getCollectiveTrainingHoursMonthlySummaryLast12Months(referenceDate = new Date()) {
    const pool = await (0, db_1.getPool)();
