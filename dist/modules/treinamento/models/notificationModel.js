...
exports.generateNormaExpiryNotifications = generateNormaExpiryNotifications;
exports.listNormaExpiryNotifications = listNormaExpiryNotifications;
exports.markNormaExpiryNotificationAsRead = markNormaExpiryNotificationAsRead;
...
async function generateNormaExpiryNotifications(lookaheadDays = 7) {
    const normalizedLookahead = Math.max(1, Math.trunc(lookaheadDays));
    if (!(await hasNormaValidityColumns())) {
        return 0;
    }
    await (0, userTrainingModel_1.archiveExpiredNormaTrainings)();
    const pool = await (0, db_1.getPool)();
    const result = await pool
        .request()
        .input("LOOKAHEAD_DAYS", db_1.sql.Int, normalizedLookahead)
        .query(`
...
async function listNormaExpiryNotifications(status = "pendente") {
    const pool = await (0, db_1.getPool)();
...
async function markNormaExpiryNotificationAsRead(id, read) {
    const pool = await (0, db_1.getPool)();
    const result = await pool
        .request()
