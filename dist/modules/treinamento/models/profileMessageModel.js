...
exports.listProfileMessages = listProfileMessages;
exports.createProfileMessage = createProfileMessage;
exports.getProfileMessageById = getProfileMessageById;
exports.setProfileMessageReaction = setProfileMessageReaction;
...
const normalizeUsernameValue = (value) => String(value ?? "")
    ...
    .split("@")[0]
    ?.trim() ?? "";
let ensureProfileMessageSchemaPromise = null;
function mapProfileMessageRecord(record) {
    return {
        id: record.ID,
...
async function listProfileMessages(params) {
    ...
    const result = await pool
        .request()
        .input("PERFIL_USERNAME", db_1.sql.VarChar(255), profileUsername)
        .input("VIEWER_USERNAME", db_1.sql.VarChar(255), viewerUsername || null)
        .query(`
      SELECT
        m.ID,
        m.PERFIL_USERNAME,
...
async function createProfileMessage(input) {
    await ensureProfileMessageSchema();
    ...
    if (parentId) {
        ...
        const parentResult = await pool
            .request()
            .input("ID", db_1.sql.UniqueIdentifier, parentId)
            .input("PERFIL_USERNAME", db_1.sql.VarChar(255), profileUsername)
            .query(`
        SELECT TOP 1 ID
    ...
    await pool
        ...
        .input("PERFIL_USERNAME", db_1.sql.VarChar(255), profileUsername)
        .input("PARENT_ID", db_1.sql.UniqueIdentifier, parentId)
        ...
        .input("CONTEUDO", db_1.sql.NVarChar(db_1.sql.MAX), content)
...
async function getProfileMessageById(messageId) {
    await ensureProfileMessageSchema();
    ...
    const result = await pool
        .request()
        .input("ID", db_1.sql.UniqueIdentifier, normalizedMessageId)
        .query(`
      SELECT TOP 1
        ID,
        PERFIL_USERNAME,
...
async function setProfileMessageReaction(input) {
    await ensureProfileMessageSchema();
    ...
    if (!reaction) {
        await pool
            .request()
            .input("MENSAGEM_ID", db_1.sql.UniqueIdentifier, messageId)
            .input("USUARIO_USERNAME", db_1.sql.VarChar(255), username)
            .query(`
    ...
    await pool
        ...
        .input("MENSAGEM_ID", db_1.sql.UniqueIdentifier, messageId)
        .input("USUARIO_USERNAME", db_1.sql.VarChar(255), username)
        .input("TIPO", db_1.sql.VarChar(10), reaction)
        .input("REAGIDO_EM", db_1.sql.DateTime2, reactedAt)
        .query(`
