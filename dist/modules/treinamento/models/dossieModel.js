...
exports.getDossieDataByCpf = getDossieDataByCpf;
exports.listDossieCandidates = listDossieCandidates;
const db_1 = require("../config/db");
const userTrainingModel_1 = require("./userTrainingModel");
const userFaceModel_1 = require("./userFaceModel");
...
async function listDossieRowsByType(params) {
  ...
  `);
    const hasFaceEvidenceColumns = Boolean(trainingFaceColumns.recordset[0]?.HAS_FACE_BASE64) &&
    ...
    const request = pool
        ...
        .input("RELATION_TYPE", db_1.sql.VarChar(20), params.relationType);
...
async function getDossieDataByCpf(cpf) {
    ...
    const [user, latestFace, normasRows, procedimentosRows] = await Promise.all([
        (0, userModel_1.getUserByCpf)(cpf),
        (0, userFaceModel_1.getLatestUserFaceByCpf)(cpf),
...
async function listDossieCandidates() {
    await (0, userTrainingModel_1.archiveExpiredNormaTrainings)();
    const pool = await (0, db_1.getPool)();
