...
exports.listUserTrainingStatusReport = listUserTrainingStatusReport;
exports.listObraTrainingStatusReport = listObraTrainingStatusReport;
exports.listProcedimentoVersionChangesReport = listProcedimentoVersionChangesReport;
exports.listObraTrainingOverviewReport = listObraTrainingOverviewReport;
...
`;
async function listUserTrainingStatusReport(cpf) {
...
async function listObraTrainingStatusReport(params) {
    const pool = await (0, db_1.getPool)();
    const request = pool.request();
    if (params?.obraNome && params.obraNome.trim()) {
        request.input("OBRA_NOME", db_1.sql.NVarChar(255), params.obraNome.trim());
    }
    else {
        request.input("OBRA_NOME", db_1.sql.NVarChar(255), null);
    }
    const onlyPending = params?.apenasPendentes === true;
    const onlyCompleted = params?.apenasConcluidos === true;
    const statusFilters = ["sa.TOTAL_VIDEOS > 0"];
    if (onlyPending) {
...
async function listProcedimentoVersionChangesReport(params) {
    const pool = await (0, db_1.getPool)();
    const request = pool.request();
    request.input("DATA_INICIO", db_1.sql.DateTime2, params?.inicio ?? null);
    request.input("DATA_FIM", db_1.sql.DateTime2, params?.fim ?? null);
  ...
  `);
    return {
        alteracoes: changedResult.recordset,
        ativos: activeResult.recordset,
    };
}
async function listObraTrainingOverviewReport() {
    const pool = await (0, db_1.getPool)();
